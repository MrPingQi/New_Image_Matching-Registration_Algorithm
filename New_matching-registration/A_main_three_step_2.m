...
[I1_s,I1_o] = Preproscessing(image_1,resample1,[]); % resample: one may pre-resample the images to avoid unnecessary burdens
[I2_s,I2_o] = Preproscessing(image_2,resample2,[]); % []: one may select 1 or 3 bands for display, eg.[8,4,3]
...
batchsize = 500;
[M,N] = size(I1);
batch_x = round(N/batchsize);
batch_y = round(M/batchsize);
...
for j=1:batch_x
    if j==batch_x
        x1 = (j-1)*batchsize; x2 = N;
    else
        x1 = (j-1)*batchsize; x2 = x1+batchsize;
    end
    for i=1:batch_y
        if i==batch_y
            y1 = (i-1)*batchsize; y2 = M;
        else
            y1 = (i-1)*batchsize; y2 = y1+batchsize;
        end
        n = n+1;
        ...
        if size(cor1_t,1)>20
            ...
            solution_batch = solution...
                *[1,0,0;0,1,0;-dX,-dY,1]*(t_form.T)*[1,0,0;0,1,0;dX,dY,1];
            [I2_r_t,I2_rs_t] = Transform_ref_1_cut...
...
cor2(:,3) = 1; cor2 = cor2/solution;
matchment = Show_Matches(I1_o,I2_o,cor1,cor2,1);pause(0.01)
I1_s = Visual(I1_s);
I2_rs = Visual(I2_rs);
I3 = I1_s/2 + I2_rs/2;
grid_num = 10;
